...
export function useNotification() {
    ...
    const {member, getMemberByCookie} = useAuthStore();
    ...
    useEffect(() => {
        ...
        const stompClient = new Client({
            ...
            webSocketFactory: () => new WebSocket(process.env.NEXT_PUBLIC_WEBSOCKET_URL!),
            onConnect: () => {
                console.log("Connected to WebSocket");
                stompClient.subscribe(`/notifications/${member.id}`, (message) => {
                    console.log("subscribed to notifications: ", `notifications/${member.id}`);
